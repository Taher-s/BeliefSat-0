...
#define SATCALL_SIGN "VU3CDI"   //need to change
...
#define PTT_VHF (3)                         //need to define
...
float get_Temperature(int thermistor_output){
  ...
  thermistor_adc_val = analogRead(A2);
//  add ADMUX =0 as per rohit 
  ADMUX = 0;
...
void send_telemetry(){
  //APRS_setMessageDestination("NOCALL", 0);
  wdt_reset();
  float float_temp1=  get_Temperature(THERM_1);        //16 is the pin number from where we get analog input
...
//  telemetry as per Rohit Bokade
  ...
  String wd_lsb = String(wd_induced_resets_lsb);
  String wd_msb = String(wd_induced_resets_msb);
//telemetry as per Rohit Bokade
  String telemetry = "t#" + sss + "," + resets_msb + "," + wd_lsb + "," + wd_msb + "," + temp1 + "," + temp2 + ","+ "xxxxxx" +  "BeliefSat-0";
 // String telemetry = " Total Resets :" + watchdog_resets_no + " Dorgi temp 1: " + temp1 +  " Payload temp 2: " + temp2;
...
void DRA_V_config(String Rx_freq, String Tx_freq) {
  vhf_port.begin(9600);
  // Set DRA818V frequency and CTCSS tone
